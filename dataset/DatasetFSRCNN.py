...
from .utils_data import hrs_to_lrs, ycbcrs_to_patches




class DatasetFSRCNN(Dataset):
    ...
    def __init__(self, args):
        ...
        self.in_patches, self.hr_patches = (self.lr_ycbcrs, self.hr_ycbcrs) if self.phase=='test' \
            else ycbcrs_to_patches(self.lr_ycbcrs, self.hr_ycbcrs, self.patch_size, self.stride, self.scale)
...
def main(json_path = 'Implementation.json'):
    ...
    for phase, dataset_opt in args['dataset'].items():
        if phase == 'train':
            train_set = DatasetFSRCNN(dataset_opt)
            train_dataloader = DataLoader(train_set, batch_size=64, shuffle=True)
        elif phase == 'test':
            test_set = DatasetFSRCNN(dataset_opt)
            test_dataloader = DataLoader(test_set, batch_size=1, shuffle=True)
        else:
            raise NotImplementedError("Phase [%s} is not recognized."%phase)

    for i, train_data in enumerate(train_dataloader):
        print(i, 'in', train_data['in'].shape)
        print(i, 'hr', train_data['hr'].shape)
    for j, test_data in enumerate(test_dataloader):
        print(j, 'in', test_data['in'].shape)
        print(j, 'hr', test_data['hr'].shape)
